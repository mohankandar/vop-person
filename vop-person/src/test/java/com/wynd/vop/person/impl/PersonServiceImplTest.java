...
*/
  ...
  public void testSampleFindByParticipantID_Cached() {
    ...
    public void testSampleFindByParticipantID_UnCached() {
        ...
        expectedServiceMessageKeys.add( PersonMessageKeys.VOP_SAMPLE_SERVICE_IMPL_RESPONDED_WITH_MOCK_DATA.getKey());
        expectedServiceMessageKeys.add( PersonMessageKeys.VOP_SAMPLE_SERVICE_DATABASE_CALL_PERFORMED.getKey()); // Add for the DB component branch
        expectedServiceMessageKeys.add( PersonMessageKeys.VOP_SAMPLE_SERVICE_IMPL_RESPONDED_WITH_PARTNER_MOCK_DATA.getKey()); // Add for the Partner component branch
    ...
    public void testSampleFindByParticipantID_UnCached_NoData() {
        ...
        Set<String> expectedServiceMessageKeys = new HashSet<>();
        expectedServiceMessageKeys.add( PersonMessageKeys.VOP_SAMPLE_SERVICE_IMPL_RESPONDED_WITH_MOCK_DATA.getKey());
        expectedServiceMessageKeys.add( PersonMessageKeys.VOP_SAMPLE_SERVICE_DATABASE_CALL_RETURNED_NULL.getKey()); // Add for the DB component branch
        expectedServiceMessageKeys.add( PersonMessageKeys.VOP_SAMPLE_SERVICE_IMPL_RESPONDED_WITH_PARTNER_MOCK_DATA.getKey()); // Add for the Partner component branch
    ...
    public void testSampleFindByParticipantID_UnCached_Exception() {
        // Setup
        SampleDomainRequest sampleDomainRequest = createSampleDomainRequest();
        VopRuntimeException expectedRuntimeException = new VopRuntimeException(MessageKeys.VOP_GLOBAL_GENERAL_EXCEPTION,
    ...
    public void testSampleFindByParticipantID_Exception() {
        ...
        // Verifications
        verify(mockVopLogger).error(EXCEPTION_MESSAGE, expectedRuntimeException);
        assertNotNull(response);
        assertNotNull(response.getSampleInfo());
        assertEquals("JANE DOE", response.getSampleInfo().getName());
        assertEquals(PARTICIPANT_ID, response.getSampleInfo().getParticipantId());
